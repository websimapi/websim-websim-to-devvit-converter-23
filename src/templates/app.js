...
// User identity endpoint for client-side hotswap of username & avatar
router.get('/api/user-info', async (_req, res) => {
    try {
        const user = await reddit.getCurrentUser(); // undefined if logged out
        let snoovatarUrl = null;

        if (user?.username) {
            try {
                snoovatarUrl = await reddit.getSnoovatarUrl(user.username);
            } catch (e) {
                console.warn('Failed to get snoovatar URL, falling back to profile image', e);
                snoovatarUrl = user.snoovatarImage ?? null;
            }
        }
        ...
        console.error('User info endpoint error:', e);
        res.status(500).json({ error: e.message });
    }
});
