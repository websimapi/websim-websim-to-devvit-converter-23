        ...
        init: async () => {
            ...
            try {
                ...
                if (data.dbData) {
                    ...
                    // Legacy stub support
                    if (window.WebsimSocket && window.WebsimSocket.updateIdentity && window._currentUser) {
                ...
                // Hot-swap identity with live Reddit username + Snoovatar
                try {
                    ...
                    if (resp.ok) {
                        ...
                        if (info && info.username) {
                            const avatarUrl = info.snoovatarUrl || 'https://www.redditstatic.com/avatars/avatar_default_02_FF4500.png';
                            const existing = window._currentUser || {};
                            const updatedUser = {
                                ...existing,
                                username: info.username,
                                avatar_url: avatarUrl
                            };
                            window._currentUser = updatedUser;

                            if (window.WebsimSocket && window.WebsimSocket.updateIdentity) {
                                window.WebsimSocket.updateIdentity(updatedUser);
                            }
                        }
                    }
                } catch (e) {
                    console.warn("[Bridge] /api/user-info fetch failed; using fallback identity", e);
                }
            } catch (e) {
